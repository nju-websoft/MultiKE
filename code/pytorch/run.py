...
def run_itc(args):
    ...
    model = MultiKENet(data.kgs.num_entities, data.kgs.num_relations, data.kgs.num_attributes, args.embed_dim, data.value_vectors, data.local_name_vectors)
    model.to(device)
    ...
    out_folder = generate_out_folder(args.output, args.training_data, '', 'MultiKECV')
    early_stop = False
...
def run_ssl(args):
    ...
    device = torch.device(args.device)
    model = MultiKENet(data.kgs.entities_num, data.kgs.relations_num, data.kgs.attributes_num, args.embed_dim, data.value_vectors, data.local_name_vectors, True)
    model.to(device)
    ...
    out_folder = generate_out_folder(args.output, args.training_data, '', 'MultiKELate')
    early_stop = False
